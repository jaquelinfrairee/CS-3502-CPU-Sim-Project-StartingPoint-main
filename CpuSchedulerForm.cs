...
4. Compare algorithm behaviors and outcomes";
        ...
        private List<SchedulingResult> RunSRTF(List<ProcessData> processes)
        {
            var results = new List<SchedulingResult>();
            int time = 0;
            ...
            while (completed.Count < processes.Count)
            {
                var available = processes
                    .Where(p => p.ArrivalTime <= time && !completed.Contains(p.ProcessID))
                ...
                remaining[available.ProcessID]--;
                time++;

                if (remaining[available.ProcessID] == 0)
                {
                    completed.Add(available.ProcessID);

                    int finish = time;
                    int turnaround = finish - available.ArrivalTime;
                    int waiting = turnaround - available.BurstTime;

                    results.Add(new SchedulingResult
                    {
                        ProcessID = available.ProcessID,
                        ArrivalTime = available.ArrivalTime,
                        BurstTime = available.BurstTime,
                        FinishTime = finish,
        ...
        private List<SchedulingResult> RunHRRN(List<ProcessData> processes)
        {
            var results = new List<SchedulingResult>();
            int time = 0;

            var remaining = new List<ProcessData>(processes);

            while (remaining.Count > 0)
            {
                var available = remaining
                    .Where(p => p.ArrivalTime <= time)
                    .ToList();
                ...
                var next = available
                    .OrderByDescending(p =>
                        ((time - p.ArrivalTime) + p.BurstTime) / (double)p.BurstTime)
                    .First();

                int start = time;
                int finish = start + next.BurstTime;

                int turnaround = finish - next.ArrivalTime;
                int waiting = turnaround - next.BurstTime;
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            double cpuUtil = (totalBurst / totalTime) * 100;
            double throughput = results.Count / totalTime;
            ...
            summaryItem.SubItems.Add($"CPU: {cpuUtil:F1}%");
            summaryItem.SubItems.Add($"Throughput: {throughput:F2}");
        ...
        private void FirstComeFirstServeButton_Click(object sender, EventArgs e)
        {
            ...
            if (processData.Count > 0)
            {
                ...
                DisplaySchedulingResults(results, "FCFS");
